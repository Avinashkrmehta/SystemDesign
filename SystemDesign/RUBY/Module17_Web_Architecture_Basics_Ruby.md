...
> **Ruby Context:** Ruby on Rails handles the traditional request-response cycle out of the box. For WebSockets, Rails provides **Action Cable**, which integrates WebSocket support directly into the framework. For SSE, you can use `ActionController::Live`. Libraries like `faye-websocket` and `async` also provide WebSocket and evented I/O support outside of Rails.
...
> **Ruby Context:** Rails uses cookie-based sessions by default (encrypted and signed). For external session stores, the `redis-session-store` gem or `ActiveRecord::SessionStore` are common choices. The `jwt` gem provides JWT token support. Rails makes it straightforward to switch session stores via configuration in `config/session_store.rb`.
...
> **Ruby Context:** Ruby on Rails is one of the most well-known thin-client / SSR frameworks. Rails uses ERB (Embedded Ruby), Haml, or Slim templates to render HTML on the server. The recent addition of **Hotwire (Turbo + Stimulus)** allows Rails to deliver SPA-like interactivity while keeping the server-rendered approach — sending HTML over the wire instead of JSON.
...
> **Ruby Context:** Rails' **Hotwire** stack (Turbo Drive, Turbo Frames, Turbo Streams + Stimulus) is a modern hybrid approach that gives SPA-like speed without a JavaScript framework. Turbo Drive intercepts link clicks and form submissions, replacing only the changed parts of the page. This is Rails' answer to the SSR vs SPA debate — keeping the simplicity of server rendering with the responsiveness of a SPA.
...
- Modules communicate via **method calls** (in-process, not over the network)
- Single **shared database** for all modules
- Deployed as a **single artifact** (Docker image, Heroku slug, Capistrano release)
- Scaled by running **multiple copies** of the entire application behind a load balancer

> **Ruby Context:** A typical Rails monolith includes models, controllers, views, mailers, jobs, and channels all in one application. Deployment is commonly done via **Capistrano**, **Kamal** (Docker-based deployment from the Rails team), or platform services like **Heroku**. The application server (Puma, Unicorn) runs the entire app as a single process (or set of worker processes).
...
| **Low latency** | Module-to-module calls are in-process method calls (nanoseconds, not milliseconds) |
...
> **Ruby Context:** Rails supports modular monoliths through **Rails Engines** — self-contained mini-applications that can be mounted inside a host app. Each engine can have its own models, controllers, routes, and migrations. Gems like `packwerk` (by Shopify) enforce module boundaries and prevent cross-module coupling. Shopify's own monolith is a famous example of a modular Rails monolith at massive scale.
...
| Latency | Nanoseconds (method call) | Milliseconds (network) |
...
> **Ruby Context:** When extracting services from a Rails monolith, common patterns include: using **Rails Engines** as an intermediate step (modularize first, then extract), exposing internal APIs via `grape` or `rails-api`, and using **Sidekiq** or **Kafka** for async communication between the monolith and new services. Shopify's `packwerk` tool helps identify module boundaries before extraction.
...
| Modules with different technology needs | ML recommendation engine needs Python; core API is in Ruby |
...
> **Ruby Context:** In Rails, the three tiers map naturally: **Controllers** (presentation), **Models + Service Objects** (business logic), and **ActiveRecord** (data access/ORM). While Rails' "convention over configuration" blurs these boundaries by default (models often contain both business logic and data access), well-structured Rails apps use service objects (`app/services/`), form objects, and the Repository pattern to maintain clean separation.
...
class UsersController < ApplicationController
  before_action :authenticate_request  # verify identity

  def show
    user_id = params[:id]

    # Format validation
    unless valid_uuid?(user_id)
      render json: { error: "Invalid user ID" }, status: :bad_request
      return
    end

    # Delegate to business layer (service object)
    user = UserService.new.find_user(user_id)
...
```ruby
# Business Logic Layer (Service Object)
class OrderService
  ...
  def create_order(user_id:, items:)
    # Business validation
    raise BusinessError, "Order must have at least one item" if items.empty?

    total = calculate_total(items)
    raise BusinessError, "Order total must be positive" if total <= 0

    # Business workflow
    @inventory_service.reserve(items)                          # Step 1: Reserve inventory
    payment = @payment_service.charge(user_id, total)          # Step 2: Charge
    order = @order_repo.save(
      Order.new(user_id: user_id, items: items,
                total: total, payment_id: payment.id)
    )                                                          # Step 3: Save
    @notification_service.send_order_confirmation(order)       # Step 4: Notify
...
> **Ruby Context:** Service objects are a widely adopted pattern in Rails to keep controllers thin and models focused on persistence. Gems like `interactor`, `dry-transaction`, and `trailblazer` provide structured approaches to organizing business logic. For complex workflows, **Sidekiq** can handle async steps (like sending notifications) via background jobs.
...
class OrderRepository
  ...
  def update_status(order_id, status)
    Order.where(id: order_id).update_all(status: status, updated_at: Time.current)
  end
end
```

> **Ruby Context:** ActiveRecord is Rails' built-in ORM and serves as the data access layer. While ActiveRecord combines the data access and domain model into one class (Active Record pattern), you can layer a Repository pattern on top for cleaner separation. Connection pooling is handled automatically by ActiveRecord's connection pool (configured in `database.yml`). For raw SQL, use `ActiveRecord::Base.connection.execute`.
...
|-------|----------|---------|
| **Entities** | Core business objects and rules; independent of any framework | `Order`, `User`, `Payment` domain objects |
...
| **Frameworks & Drivers** | External tools and frameworks | Rails, PostgreSQL adapter, React, gRPC server |

> **Ruby Context:** The `dry-rb` ecosystem (`dry-system`, `dry-container`, `dry-monads`) and **Hanami** (a Ruby web framework) are designed with Clean Architecture principles in mind. Hanami explicitly separates entities, repositories, and actions (use cases). In Rails, you can achieve Clean Architecture by using service objects, form objects, and the Repository pattern — though it requires more discipline since Rails conventions favor the Active Record pattern.
...
> **Ruby Context:** In Ruby, ports are typically defined as modules or abstract base classes (using `NotImplementedError`), and adapters are concrete classes that implement them. Duck typing makes this natural — any object that responds to the expected methods can serve as an adapter. The `dry-container` gem provides dependency injection to wire adapters to ports at runtime.
...
> **Ruby Context:** In production Ruby deployments, a reverse proxy (typically **Nginx**) sits in front of the Ruby application server (**Puma** or **Unicorn**). Nginx handles SSL termination, static file serving, and request buffering, while Puma/Unicorn handles dynamic Ruby requests. This is the standard production architecture for Rails applications. The **Rack** middleware stack also acts as a lightweight internal proxy chain within the Ruby app itself.
...
upstream app_servers {
    ...
    server 10.0.1.10:3000 weight=3;     # Higher weight = more traffic (Puma instances)
    server 10.0.1.11:3000 weight=2;
    server 10.0.1.12:3000 weight=1;     # Canary server (less traffic)
...
> **Ruby Context:** Note the upstream servers use port `3000` (Puma's default) instead of `8080`. The WebSocket location is `/cable` — the default Action Cable endpoint in Rails. In a typical Rails + Nginx + Puma setup, Nginx can also be configured to serve assets from `public/` directly (via `location /assets/`), bypassing Puma entirely for static files. Tools like **Kamal** (formerly MRSK) automate this Nginx + Puma configuration for Docker-based Rails deployments.
...
> **Ruby Context:** Rails integrates with CDNs via the `config.asset_host` setting, which prefixes all asset URLs with your CDN domain. The **Asset Pipeline** (Sprockets) or **Propshaft** compiles and fingerprints assets (e.g., `application-abc123.css`), enabling aggressive CDN caching with versioned URLs. For Active Storage file uploads, Rails can be configured to serve files through a CDN using `config.active_storage.resolve_model_to_route = :cdn`.
...
> **Ruby Context:** Rails' Asset Pipeline (Sprockets) and Propshaft automatically generate fingerprinted asset filenames (e.g., `application-abc123def456.css`) based on content hashes. This is built-in cache busting — when the file content changes, the fingerprint changes, creating a new URL. Combined with `config.asset_host` pointing to a CDN, this gives you optimal caching with zero manual invalidation.
...
> **Ruby Context:** In a typical production Rails architecture, the app servers run **Puma** (threaded) or **Unicorn** (forked processes) behind the load balancer. Background job processing is handled by **Sidekiq** (backed by Redis) running on separate worker instances. The full stack often looks like: DNS → CDN → Load Balancer → Nginx → Puma (Rails) → PostgreSQL + Redis (cache/sessions/jobs).
...
> **Ruby Context:** In Ruby-focused interviews, be ready to discuss the typical Rails production stack: Nginx → Puma/Unicorn → Rails → PostgreSQL + Redis. Know how **Rack middleware** acts as an internal proxy chain, how **Action Cable** handles WebSockets, how **Sidekiq** handles background jobs, and how Rails' Asset Pipeline provides built-in cache busting for CDN integration. Understanding **Hotwire** (Turbo + Stimulus) as Rails' modern approach to the thin-client vs thick-client debate is also valuable.