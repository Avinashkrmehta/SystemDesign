...
> **Ruby Context:** Ruby on Rails applications commonly interact with databases through ActiveRecord, which abstracts many of these concepts. However, understanding the underlying mechanics is essential for scaling Rails applications beyond a single database server. Gems like `makara` (read/write splitting), `activerecord-import` (bulk operations), and `strong_migrations` (safe schema changes) help manage these concerns at the application level.
...
> **Ruby Context:** In a Rails application, your `database.yml` configuration determines which database you connect to. When using PostgreSQL with streaming replication (CP), gems like `makara` or `octopus` can route reads to replicas and writes to the primary. When using Cassandra (AP), the `cassandra-driver` gem lets you configure consistency levels per query.
...
> **Ruby Context:** ActiveRecord transactions map directly to ACID guarantees. When you use `ActiveRecord::Base.transaction { ... }`, you get atomicity and isolation from the underlying database. For distributed transactions across multiple databases, consider the `distributed_transaction` pattern or saga pattern — ActiveRecord doesn't natively support 2PC.

```ruby
# ActiveRecord ACID transaction example
ActiveRecord::Base.transaction do
  account_from.lock!  # Pessimistic locking for isolation
  account_to.lock!
  ...
  # After commit, changes survive crashes (durability)
end
```
...
> **Ruby Context:** In a Rails e-commerce app, steps 2-3 would use `ActiveRecord::Base.transaction`, while steps 4-6 would be dispatched via `ActiveJob` to a background queue (Sidekiq, GoodJob) for eventual processing. This naturally implements the hybrid ACID/BASE pattern.

```ruby
# Hybrid ACID/BASE in Rails
ActiveRecord::Base.transaction do
  # ACID: Reserve inventory and process payment
  inventory_item.lock!
  inventory_item.decrement!(:quantity)
  payment = Payment.create!(order: order, amount: total)
  order.update!(status: :confirmed)
end

# BASE: Async jobs for non-critical paths
OrderConfirmationMailer.deliver_later(order)
AnalyticsTrackingJob.perform_later(order.id)
RecommendationUpdateJob.perform_later(order.user_id)
```
...
> **Ruby Context:** Rails 6+ has built-in support for multiple databases and automatic read/write splitting. Configure primary and replica connections in `database.yml`, and Rails will route writes to the primary and reads to replicas automatically.

```ruby
# config/database.yml — Rails multi-database configuration
production:
  primary:
    adapter: postgresql
...
# Rails automatically routes reads to replica, writes to primary
User.find(123)          # → reads from replica
...
> **Ruby Context:** Rails 6+ handles this automatically with a configurable delay. After a write, Rails will read from the primary for a short window (default 2 seconds) before switching back to the replica.

```ruby
# config/application.rb — configure the delay window
config.active_record.database_selector = { delay: 2.seconds }
config.active_record.database_resolver = ActiveRecord::Middleware::DatabaseSelector::Resolver
config.active_record.database_resolver_context = ActiveRecord::Middleware::DatabaseSelector::Resolver::Session

# You can also force reads from primary explicitly
ActiveRecord::Base.connected_to(role: :writing) do
  # This read goes to the primary, not the replica
...
> **Ruby Context:** You can monitor replication lag from your Rails application and make routing decisions based on it:

```ruby
# Check replication lag in a Rails app (PostgreSQL)
class ReplicationMonitor
  def self.lag_bytes
    ...
    result.first&.dig("lag_bytes").to_i
  end

  def self.safe_to_read_from_replica?(max_lag_bytes: 1_048_576) # 1MB
...
> **Ruby Context:** Rails doesn't have built-in sharding support at the query level, but Rails 6.1+ introduced horizontal sharding configuration. Gems like `activerecord-sharding`, `octopus`, and middleware like Vitess or Citus (for PostgreSQL) are commonly used. For application-level sharding, you configure multiple database connections and route queries based on the shard key.
...
> **Ruby Context:** Rails 6.1+ supports horizontal sharding natively. You define shards in `database.yml` and use `connects_to` with shard mappings:
...
production:
  primary:
    adapter: postgresql
    host: shard-default.example.com
    database: myapp_shard_default
  primary_shard_one:
    adapter: postgresql
    host: shard-one.example.com
    database: myapp_shard_one
  primary_shard_two:
    adapter: postgresql
    host: shard-two.example.com
...
class ShardRouter
  def self.shard_for(user_id)
    case user_id % 3
    when 0 then :default
    when 1 then :shard_one
    when 2 then :shard_two
    end
  end
end

# Usage
shard = ShardRouter.shard_for(user_id)
...
> **Ruby Context:** Implementing scatter-gather in Ruby:

```ruby
# Cross-shard scatter-gather in Rails
class CrossShardQuery
  SHARDS = %i[default shard_one shard_two].freeze

  def self.top_orders(min_total:, limit: 10)
    results = SHARDS.flat_map do |shard|
      ActiveRecord::Base.connected_to(shard: shard) do
        Order.where("total > ?", min_total)
             .order(created_at: :desc)
             .limit(limit)
             .to_a  # materialize the query on this shard
      end
    end

    # Merge and sort results from all shards
    results.sort_by { |o| -o.created_at.to_i }.first(limit)
  end
end

# For parallel execution across shards, use threads:
...
> **Ruby Context:** The `strong_migrations` gem helps with safe schema changes during resharding by catching dangerous migrations before they run. For zero-downtime resharding, consider using Vitess (which Shopify uses for their massive Rails application) or Citus for PostgreSQL.
...
> **Ruby Context:** You can implement a simple Bloom filter in Ruby for application-level caching or deduplication. The `bloomer` and `bloom-filter` gems provide ready-made implementations:

```ruby
# Using the bloomer gem for application-level Bloom filters
require 'bloomer'

# Create a Bloom filter expecting 1 million items with 1% false positive rate
filter = Bloomer.new(1_000_000, 0.01)
...
# Check membership
filter.include?("user:123")  # => true (definitely or probably in set)
filter.include?("user:789")  # => false (definitely NOT in set)
filter.include?("user:999")  # => possibly true (false positive ~1% of the time)

# Useful for: "Should I even query the database for this key?"
def find_user(id)
  key = "user:#{id}"
  return nil unless @bloom_filter.include?(key)  # Skip DB query if definitely not present
  User.find_by(id: id)
end
```
...
> **Ruby Context:** ActiveRecord has its own connection pool (configured via `pool` in `database.yml`) which manages database connections, not to be confused with the database's buffer pool. For Rails applications, ensure both are properly tuned:

```ruby
# config/database.yml — connection pool configuration
production:
  adapter: postgresql
  pool: <%= ENV.fetch("RAILS_MAX_THREADS") { 5 } %>
  timeout: 5000
  # The database's buffer pool (shared_buffers, innodb_buffer_pool_size)
  # is configured on the database server itself, not in Rails.

# Check connection pool status in Rails
...
> **Ruby Context:** You can use `EXPLAIN` from ActiveRecord to inspect query plans:

```ruby
# View the query plan for an ActiveRecord query
User.joins(:orders)
    .where(status: "active")
    .group(:name)
...
# Or use the bullet gem to detect N+1 queries and missing indexes
# Gemfile
...
> **Ruby Context:** After running large data migrations in Rails, it's good practice to run `ANALYZE` on affected tables. You can add this to your migration:

```ruby
class BackfillUserStatuses < ActiveRecord::Migration[7.1]
  def up
    # Bulk update
    User.where(status: nil).update_all(status: "active")

    # Update statistics after bulk change
...
> **Ruby Context:** Rails applications typically serve as the OLTP layer. For OLAP, you'd extract data from your Rails database into a data warehouse. Gems like `scenic` (for database views) and `groupdate` (for time-based grouping) can help with lightweight analytics directly in Rails, but heavy analytics should go to a dedicated warehouse.
...
> **Ruby Context:** Ruby is commonly used for ETL scripts and data pipelines. The `kiba` gem is a popular Ruby ETL framework, and `ruby-kafka` can stream data to Kafka for real-time pipelines:

```ruby
# Simple ETL pipeline using Kiba
require 'kiba'

# Define the ETL job
job = Kiba.parse do
  # Extract: Read from the Rails database
  ...
  transform do |order|
    {
      ...
      order_date: order.created_at.to_date.to_s
    }
  end

  # Transform: Filter out test orders
  transform { |row| row[:total_amount] > 0 ? row : nil }

  # Load: Write to the data warehouse
  destination WarehouseDestination, table: "fact_orders"
end
...
> **Ruby Context:** The `duckdb` gem provides Ruby bindings for DuckDB, allowing you to run OLAP queries directly from Ruby without a separate warehouse for smaller datasets:
...
# Load a Parquet file and query it with SQL
con.execute("CREATE TABLE sales AS SELECT * FROM read_parquet('sales_data.parquet')")
...
result.each do |row|
  puts "#{row[0]}: $#{row[1]} (#{row[2]} customers)"
end
```
...
> **Ruby Context Summary:** Rails 6+ provides built-in support for multiple databases (read/write splitting, horizontal sharding). Key gems for database scaling: `makara` (connection proxy), `strong_migrations` (safe migrations), `bullet` (N+1 detection), `scenic` (database views), `kiba` (ETL), `bloomer` (Bloom filters), and `duckdb` (embedded OLAP). For production sharding at scale, consider Vitess (used by Shopify) or Citus for PostgreSQL.
...
> - Know how Rails 6+ handles multiple databases and read/write splitting (`connects_to`, `connected_to`)
> - Be able to explain how `ActiveRecord::Base.transaction` maps to ACID properties
> - Understand ActiveRecord connection pooling vs database buffer pools — they're different things
> - Know when to use background jobs (Sidekiq/GoodJob) for BASE-style eventual consistency
> - Be familiar with `strong_migrations` for safe schema changes in production
> - Understand how Shopify scales Rails with Vitess for database sharding