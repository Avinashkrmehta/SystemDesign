...
**Ruby/Rails Context:** Rails uses encrypted cookies for session storage by default (`config.session_store :cookie_store`). For distributed systems, you can switch to Redis-backed sessions (`config.session_store :redis_store`). The `rack` middleware handles cookie parsing and session management transparently.
...
**Ruby Context:** Ruby's `OpenSSL` module provides TLS support. The `net/http` standard library uses it for HTTPS connections. In production Ruby applications, TLS termination is typically handled by a reverse proxy (Nginx, HAProxy) in front of the application server (Puma, Unicorn).
...
**Ruby/Rails Context:** In Rails, CORS is typically handled by the `rack-cors` gem, which provides Rack middleware for configuring allowed origins, methods, and headers.
...
**Ruby Context:** Ruby's `Net::HTTP` supports persistent connections via `Net::HTTP.start`. Application servers like Puma use persistent connections and thread pooling to handle concurrent requests efficiently.
...
**Ruby Context:** Rails provides built-in WebSocket support through **Action Cable**, which integrates WebSockets with the rest of the Rails framework. Action Cable uses Redis as a pub/sub backend for broadcasting messages across multiple server processes. For non-Rails Ruby applications, the `faye-websocket` and `websocket-driver` gems provide WebSocket client and server implementations.
...
**Ruby Context:** Action Cable in Rails uses Redis Pub/Sub by default for horizontal scaling. When running multiple Puma workers or multiple servers, Redis ensures that messages broadcast on one server reach WebSocket clients connected to other servers. For high-scale WebSocket applications, consider AnyCable — a Ruby library that offloads WebSocket handling to a Go-based server for significantly better performance and lower memory usage.
...
**Ruby Context:** Ruby's standard library includes `net/ftp` for FTP client operations. For SFTP, the `net-sftp` gem (built on `net-ssh`) is the standard choice. For modern file storage, the `aws-sdk-s3` gem or `shrine`/`carrierwave` gems with cloud storage backends are preferred.
...
**Ruby Context:** Ruby's standard library includes `net/smtp` for sending email. The `mail` gem provides a higher-level interface for composing and sending emails. In Rails, **Action Mailer** abstracts email sending entirely, supporting SMTP, Sendmail, and third-party services. For receiving/processing email, Action Mailbox (Rails 6+) provides inbound email routing.
...
**Ruby Context:** The `mqtt` gem provides a Ruby client for connecting to MQTT brokers. For IoT backends built in Ruby, you can subscribe to MQTT topics and process messages within a Ruby application, though high-throughput IoT ingestion is more commonly handled by Go or Rust services.
...
**Ruby Context:** The `bunny` gem is the most popular Ruby client for RabbitMQ/AMQP. For background job processing in Ruby, **Sidekiq** (Redis-backed) and **Sneakers** (RabbitMQ-backed) are common choices. Rails applications often use Active Job as an abstraction layer over these queue backends.
...
## Ruby Ecosystem — Networking Libraries Quick Reference

| Protocol / Use Case | Ruby Library / Gem | Notes |
|---------------------|--------------------|-------|
| HTTP Client | `net/http` (stdlib), `faraday`, `httparty`, `typhoeus` | `faraday` is the most flexible with middleware support |
| HTTP Server | Puma, Unicorn, Falcon | Puma is the default Rails server; Falcon supports HTTP/2 |
| WebSocket Server | Action Cable (Rails), `faye-websocket`, AnyCable | AnyCable offloads to Go for high performance |
| WebSocket Client | `faye-websocket`, `websocket-client-simple` | For connecting to external WebSocket services |
| TCP/UDP Sockets | `socket` (stdlib), `eventmachine` | Low-level networking; EventMachine for async I/O |
| DNS | `resolv` (stdlib) | Pure Ruby DNS resolver |
| FTP | `net/ftp` (stdlib) | Basic FTP client |
| SFTP | `net-sftp` | Built on `net-ssh` |
| SMTP | `net/smtp` (stdlib), `mail` gem, Action Mailer | Action Mailer for Rails applications |
| MQTT | `mqtt` gem | MQTT client for IoT integration |
| AMQP / RabbitMQ | `bunny` | Most popular Ruby AMQP client |
| gRPC | `grpc` gem | Google's official Ruby gRPC implementation |
| SSL/TLS | `openssl` (stdlib) | Used internally by `net/http`, `net/smtp`, etc. |
