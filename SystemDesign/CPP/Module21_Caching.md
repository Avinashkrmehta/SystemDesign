...
| **Write-Around** | Miss after write | Low (DB only) | Eventual | Low | Write-heavy, rarely read after write |
...
**Implementation:** HashMap (O(1) lookup) + Doubly Linked List (O(1) move to front / remove from back)
...
+------------------+
    ↓
+------------------+
| Application Cache|  ← In-process cache (HashMap, Guava, Caffeine)
...
Application Process:
  ...
  |                                          |
  |  +------------------------------------+  |
  |  | In-Process Cache (HashMap/Caffeine)|  |
  |  | user:123 → {name: "Alice", ...}   |  |
...
| C++ | `std::unordered_map` + custom eviction | Manual implementation |
...
```
Request → L1 (In-Process, Caffeine) → HIT → return (fastest)
                                     → MISS
...
**Best for:** Data where freshness is critical (inventory counts, pricing, user permissions).
...
def get_user(user_id):
    ...
    if cache.set(lock_key, "1", nx=True, ex=10):  # SET if Not eXists, 10s expiry
        ...
        return get_user(user_id)  # retry (will likely hit cache now)
```
...
Background Job (every 5 minutes):
  data = db.query("SELECT * FROM homepage_data")
  cache.set("homepage_data", data)  // no TTL — never expires
```
...
jitter = random.randint(0, 600)  # 0-10 minutes of random jitter
...
```
L2 (Redis) goes down:
  Request → L1 (in-process, Caffeine) → HIT → return (still works!)
...
| LRU | Default eviction policy; evicts least recently used; O(1) with HashMap + linked list |
...
2. **Cache-aside implementation** — be ready to write pseudocode for cache-aside with TTL and invalidation
3. **Eviction policies** — explain LRU vs LFU with examples; know that LRU is the default for Redis
4. **LRU implementation** — classic interview question: implement LRU cache with O(1) get/put using HashMap + doubly linked list
