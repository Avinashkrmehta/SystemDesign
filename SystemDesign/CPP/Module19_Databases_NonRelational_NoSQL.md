...
SET cache:user:123 '{"name":"Alice","age":30}' EX 3600    -- expires in 1 hour
GET cache:user:123                                         -- returns the JSON string

-- String: Atomic counter
INCR page_views:homepage          -- atomically increment by 1 → returns new value
INCRBY page_views:homepage 10     -- atomically increment by 10

-- Hash: User profile (access individual fields without deserializing)
HSET user:123 name "Alice" age 30 email "alice@example.com"
HGET user:123 name                -- returns "Alice" (no need to fetch entire object)
HINCRBY user:123 age 1            -- atomically increment age
...
ZRANK leaderboard "player:bob"   -- bob's rank (0-indexed)
ZRANGEBYSCORE leaderboard 1000 2000  -- players with score between 1000-2000

-- Set: Mutual friends
SADD friends:alice "bob" "charlie" "dave"
SADD friends:bob "alice" "charlie" "eve"
SINTER friends:alice friends:bob  -- mutual friends → {"charlie"}

-- List: Recent activity feed (capped at 100 items)
LPUSH feed:user:123 '{"action":"liked","post":456}'
LTRIM feed:user:123 0 99         -- keep only the 100 most recent items
LRANGE feed:user:123 0 9         -- get the 10 most recent items

-- Stream: Event log
XADD events:orders * action "created" order_id "789" user_id "123"
XADD events:orders * action "paid" order_id "789" amount "99.99"
XRANGE events:orders - +         -- read all events
XRANGE events:orders 1625097600000-0 +  -- events after a timestamp
```
