...
| Insight | Implication |
|---------|------------|
| **Memory is 1000x faster than SSD** | Cache hot data in memory (Redis, in-process cache) |
...
1 μs = 0.000001 seconds = 1,000 ns
1 ms = 0.001 seconds = 1,000 μs = 1,000,000 ns
...
The number of unique users who interact with the system in a day.
...
With CDN (90% cache hit rate):
  ...
  CDN cost: much lower per GB (volume discounts, edge delivery)
  Origin cost: 10x less bandwidth → fewer/smaller servers
```
...
Example: Social media API
  Peak QPS: 100,000
  QPS per server (API + cache): 10,000
  Servers needed: 100,000 / 10,000 = 10 servers
  
  With 2x headroom for safety: 20 servers
  With 3 availability zones: ~7 servers per AZ
```
...
Example: Order database (write-heavy)
  Orders per day: 10 million
  Peak write QPS: 10M / 86,400 × 3 (peak) ≈ 350 writes/sec
  Max writes per shard: 5,000/sec (PostgreSQL)
  Shards needed (by write QPS): 350 / 5,000 = 1 shard (write QPS is fine)
  
  But after 5 years: 10M × 365 × 5 = 18.25 billion orders × 500 bytes = 9.1 TB
  Shards needed (by storage): 9.1 TB / 500 GB = ~19 shards
  
  → Storage is the bottleneck, not write QPS
```
...
Example: URL shortener
  ...
  With overhead (hash table, pointers): 4 GB × 1.5 = 6 GB
  
  → A single Redis instance (64 GB) can easily handle this!

Example: Social media timeline cache
  DAU: 100 million
  Cache each user's timeline: top 200 posts × 1 KB per post = 200 KB per user
  Cache active users (20% of DAU): 100M × 20% = 20M users
  
  Cache size: 20M × 200 KB = 4 TB
  
  → Need a Redis cluster (multiple nodes)
  → Or cache only the most active users (top 5%): 1 TB
```
...
```
Formula:
  Total connections = app_instances × connections_per_instance

Example: Database connections
  App servers: 20 instances
  Connection pool per instance: 20 connections
  Total: 20 × 20 = 400 connections
  
  PostgreSQL max_connections default: 100 → NOT ENOUGH!
  
  Solutions:
  1. Use PgBouncer: 400 client connections → 50 DB connections
  2. Increase max_connections (but each uses ~10 MB RAM)
...
| Servers | Peak QPS / QPS per server | 30K / 5K = 6 servers |
...
Storage Strategy: Text/metadata in database; media in object storage (S3).
```
...
Download bandwidth:
  2B users × 40 min × 60 sec × 5 Mbps = 2B × 2,400 × 5 Mbps
  
  Wait — not all users watch simultaneously. Assume peak concurrent viewers:
  Peak concurrent: 10% of DAU = 200 million
  
  200M × 5 Mbps = 1,000,000,000 Mbps = 1,000 Tbps = 1 Pbps (petabit per second!)
...
Upload bandwidth:
  ...
  Average upload bitrate: 10 Mbps (higher quality source)
  30,000 hours × 3,600 sec × 10 Mbps = 1,080,000,000 Mbps ≈ 1 Tbps upload
...
### Practice 4: Estimate Cache Size for URL Shortener

```
Given:
  Total URLs: 10 billion (accumulated over years)
  New URLs per day: 100 million
  Read:Write ratio: 100:1
  Read QPS: 100M × 100 / 86,400 ≈ 116,000 reads/sec
  Average URL record: 200 bytes (short code + long URL + metadata)

Cache strategy: Cache the most frequently accessed URLs
  Pareto principle: 20% of URLs account for 80% of traffic

Daily active URLs: ~500 million (5% of total — most URLs are rarely accessed)
Cache top 20% of daily active: 500M × 20% = 100 million URLs

Cache size:
  100M × 200 bytes = 20 GB

With overhead (hash table, pointers, Redis metadata):
  20 GB × 2 = 40 GB

→ A single Redis instance (64 GB RAM) handles this easily!
→ Cache hit ratio: ~80% (Pareto)
→ Database only handles 20% of reads: 116,000 × 20% = 23,200 QPS
   (manageable for a well-indexed database)
```
...
WebSocket connections:
  ...
  Memory per server: 50 million × 10 KB = 500 TB → way too much for one server!
...
8. **Server count** — Peak QPS / QPS per server; mention headroom (2x) and availability zones
9. **Shard count** — by storage (data / max per shard) or by write QPS (writes / max per shard)
10. **Cache sizing** — 20% of active data (Pareto); check if it fits in a single Redis instance
11. **Show your work** — write down assumptions, formulas, and calculations step by step
12. **Sanity check** — does the answer make sense? Compare with known systems (Twitter, YouTube, WhatsApp)
